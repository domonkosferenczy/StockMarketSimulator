...
function Animation() {
  ...
  const shownDates = allDates.slice(allDates.indexOf(state.animation.shownFrom), allDates.indexOf(currentDate)+1)
  ...
  const changeZoom = (val) => {
    if ((val > 0 && state.animation.zoom < 10) || (val < 0 && state.animation.zoom >= 2)){
      ...
      let shownFromData = dates[(dates.indexOf(currentDate)) - 12 * Math.round(Math.round(allDates.length / 12) / (state.animation.zoom + val))]
